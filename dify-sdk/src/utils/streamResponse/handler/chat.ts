 ...
 * 聊天流处理器
 */
export class ChatStreamHandler extends BaseStreamHandler<ChatStreamEvent> {
  /**
   * 注册消息事件处理器
   */
   ...
   * 注册消息文件事件处理器
   */
  onMessageFile(handler: EventHandler<MessageFileEvent>): this {
    this.registerEventHandler('message_file', handler);
    return this;
  }
